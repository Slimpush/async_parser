    ...
    for row in df_filtered.itertuples(index=False):
        try:
            parsed_data.append((
                row[0],                               # Товар (строка)
                row[1],                               # Наименование
                row[0][:4],                           # Продукт (первые 4 символа)
                row[0][4:7],                          # Тип поставки (символы с 4 по 7)
                row[2] if row[2] != '-' else None,    # Базис поставки, если есть
                row[0][-1],                           # Тип поставки (последний символ)
                int(row[3]) if row[3] != '-' else 0,  # Объём, если не '-'
                int(row[4]) if row[4] != '-' else 0,  # Итоговая сумма, если не '-'
                int(row[5]) if row[5] != '-' else 0,  # Количество сделок, если не '-'
                date                                  # Дата
            ))
